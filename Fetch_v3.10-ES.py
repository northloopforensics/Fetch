...
st.set_page_config(
   page_title="Fetch v3.10 - Español",
   #page_icon="🔴",
   layout="wide",
...
def make_geofence_map():
    ...
    help_Box = st.expander(label="Ayuda")
    with st.form("geoform"):
        user_geo_input = st.text_input("Búsqueda de Dirección y Dirección I.P.",placeholder=None,)
        search_geo_button = st.form_submit_button("Buscar")
    with help_Box:
        st.write("""Use los elementos de forma en la barra de herramientas del mapa para crear formas para la geovalla. \n\nUna vez que su geovalla haya sido dibujada, 
        las coordenadas se poblarán debajo del mapa.  \n\nPuede agregar las coordenadas a su portapapeles para pegarlas en otro lugar o 
        puede guardar toda la página usando su navegador.\n\nATENCIÓN: Las búsquedas de Protocolo de Internet NO SON INDICATIVAS DE LA UBICACIÓN DONDE SE UTILIZÓ EL IP. 
        INDICAN UN ÁREA GENERAL ASOCIADA CON EL PROVEEDOR DE SERVICIOS Y PUEDEN SER COMPLETAMENTE INEXACTAS EN ALGUNOS CASOS. \n\nVerifique 
        cualquier dirección o ubicación presentada por la barra de búsqueda. \n\nLa precisión varía según la ubicación.""")
    global geomap
    ...
    if len(user_geo_input) > 0:
        ...
        if bool(re.search(ipv4_ipv6_regex, user_geo_input)) == True:   #    searches user input for alphabet if true geocode address search - false ip address search
            ...
            zoom = 11
            st.markdown(":blue[La geolocalización de I.P. es una estimación aproximada relacionada con la cobertura del proveedor, y el punto proporcionado no indica la ubicación de uso/usuario.]")
    ...
    try:        #   pulls the lats and longs from the returned JSON encoded map points
        ...
        st.subheader("Coordenadas de la Geovallaates")
        text_of_coords = "Latitud, Longitud\n"
        for list in parse3:
            for coord in list:
                lon, lat = coord[0], coord[1]
                text_of_coords = text_of_coords + "\n" + (str(lat) + ", " + str(lon)) + "\n"
        texting = st.write(text_of_coords)
        download_coords = st.download_button(label="Descargar Coordenadas", data=text_of_coords, file_name="Fetch_GeoFence_Coordinates.txt")
...
def get_IP_locale(invalidList, IPs):    #used to map ips
    ...
    for thing in valid_only:
        ...
        if "error" in pull_geo_data.json and pull_geo_data.json.get("error") == "429":
            st.warning("Demasiadas solicitudes de búsqueda de IP han ocurrido en Fetch hoy. Por favor, inténtelo de nuevo más tarde.")
            break
        else:
...
def geo_ip_to_Dataframe(geo_list):  #used to map ips
    ...
    df.rename(columns={"IP": 'DIRECCIÓN IP','LAT': 'LATITUDE', 'LNG': "LONGITUDE", 'ORG': "PROVEEDOR DE SERVICIOS"}, inplace=True)
    return (df)
...
def make_IPaddress_Map():   #used to map ips
    ...
    user_location = st.text_input("Nombre del Lugar o Dirección - Úselo para Agregar una Ubicación Relevante al Mapa (Lugar donde se recibió el correo electrónico, punto de comparación, etc.)")
    ipdata = st.text_area("Ingrese Datos con Direcciones IP o un Encabezado de Correo Electrónico",height=200)
    ip_geo_button = st.button("Buscar")
    ...
    if ip_geo_button == True:
        ...
        try:
            ...
            show_these = ('DIRECCIÓN IP', 'STATUS','PROVEEDOR DE SERVICIOS', 'CITY', 'STATE', 'COUNTRY','LATITUDE', 'LONGITUDE')
            # show_these = None
        ...
        try:
            ...
            download_test = st.download_button(label="Descargar Mapa HTML", data=downloadfile,file_name="Fetch_Analysis_Map.html")
        except UnboundLocalError:
            pass
...
def make_map(in_df):       #bring in pandas dataframe
    try:
        ...
        map_Type = st.radio("Select Map Type", options=["Marcadores Agrupados", "Puntos y Senderos", "Mapa de Calor", "Sitios de Celulares"], horizontal=True)
        Map = leafmap.Map()
        ...
        # )
        # Map.add_tile_layer(
        #     url=f'https://api.mapbox.com/styles/v1/mapbox/outdoors-v11/tiles/{{z}}/{{x}}/{{y}}?access_token={mapbox_token}',
        #     name='Mapbox Outdoors',
        #     attribution='Map data © OpenStreetMap contributors, CC-BY-SA, Imagery © Mapbox'
        # )

        if map_Type == "Marcadores Agrupados":
        ...
        if map_Type == "Mapa de Calor":
            ...
            weight_type = st.radio("Select Weight Type", options=["Densidad de Puntos", "Densidad Ponderada"], horizontal=True)
            if weight_type == "Densidad de Puntos":
            ...
            if weight_type == "Densidad Ponderada":
                ...
                try:
                    ...
                    st.info("Los mapas de calor utilizan valores numéricos ponderados para acomodar problemas como la densidad de población. Seleccione la columna para su conjunto de datos.")


        if map_Type == "Puntos y Senderos":
            st.markdown("---")
            Map.zoom_to_gdf(gdf) 
            points_or_path = st.radio(label="Select map activity", options=["Marcadores", "Mostrar Progresión de Puntos", "Estela de Vapor"], horizontal=True)
            
            if points_or_path == "Marcadores":     #Shows markers only
                color = st.selectbox(label="Seleccionar",options=['DarkRed', 'Yellow','Pink', 'Green', 'Teal', "Blue", "White"])
                if st.checkbox("Los datos tienen información de precisión o radio"):
                    radius_value = st.selectbox(label="Radio/Huella en Metros", options=gdf.columns)
                    print(radius_value)
                    try:
                        ...
                        st.error("Por favor seleccione la columna con la información de radio o precisión.")        
            ...
            if points_or_path == "Mostrar Progresión de Puntos":      #Shows the moving path between markers
            ...
            if points_or_path == "Estela de Vapor":
                choose_datetime_column = st.selectbox("Columna de Fecha/Hora", options=gdf.columns)
                time_interval = st.radio("Intervalo de Tiempo para Mostrar", options=['Diario', 'Por Hora', '10 Minutos', '1 Minuto'],horizontal=True)
                if time_interval == 'Diario':
                    chosen_interval = 'PT24H'
                if time_interval == 'Por Hora':
                    chosen_interval = 'PT1H'
                if time_interval == '10 Minutos':
                    chosen_interval = 'PT10M'  
                if time_interval == '1 Minuto':
        ...
        if map_Type == "Sitios de Celulares":
            ...
            wedge_color = st.selectbox("Color del Sector", options=['Red', 'Blue', 'Green', 'Purple', 'Orange', 'DarkRed', 'Beige', 'DarkBlue', 'DarkGreen', 'CadetBlue', 'Pink', 'LightBlue', 'LightGreen', 'Gray', 'Black', 'LightGray'])
        
            radii_list = ["1.5 Millas", "1 Kilometer"]
            for oto in in_df.columns:
                radii_list.append(oto)
            radii = st.selectbox("Tamaño de la Forma del Sector", options=radii_list)
            if radii == "1.5 Millas":
                in_df["1.5 Millas"] = pandas.Series(2414 for x in range(len(in_df.index)))
            if radii == "1 Kilometer":
                in_df["1 Kilometer"] = pandas.Series(1000 for x in range(len(in_df.index)))
            Azimuth = st.selectbox("Azimut del Sector", options=in_df.columns)   
            beam_width = st.selectbox("Ancho del Haz del Sector", options=in_df.columns, placeholder='None')
            try:
...
def create_kml(df_in, outfile):
    try:
        ...
        st.subheader("Diseñar Mapa KML")
        icon = st.selectbox("Seleccionar Estilo de Icono", options=icon_options)        

        label_for_icons = st.selectbox("Etiquetas de Iconos del Mapa", options=headers)
        
        footprint = st.checkbox("El conjunto de datos incluye información de radio/área", value=False)
        radii = None
        if footprint:
            radii = st.selectbox("Radio/Distancia desde el Punto en Metros", options=headers)
        
        tour = st.checkbox("Incluir Recorrido KML", value=False)
        if tour:        # Tour Settings
            st.subheader("Diseñar Configuración del Recorrido")
            there_are_dates = st.checkbox("El conjunto de datos incluye información de fecha/hora", value=False)
            if there_are_dates:
                time_column = st.selectbox("Columna de Fecha/Hora", options=headers)
            else:
                time_column = None
            tour_altitude = st.selectbox("Altitud del Recorrido (Metros)", options=['50','150', '250', '300', '750', '1500', '10000'], index=2)
            tour_linger_time = st.selectbox("Tiempo de Permanencia del Recorrido (Segundos))", options=['1', '2', '3', '4', '5', '10', '15'], index=3)
            tour_tilt = st.selectbox("Inclinación del Recorrido", options=['0', '5', '10', '20'], index=1)

    except AttributeError:
        print("Attribute error on variable headers in createkml")
        st.error("Check for errors in column selection.")

    if st.button("Crear KML"):
        ...
        if not filename:
            ...
            if not tour:
                ...
                st.download_button("Descargar KML", data=kml_data, file_name="Fetch_KML_Download.kml")
            ...
            if tour:
                ...
                st.download_button("Descargar Recorrido KML", data=tour_data, file_name="Fetch_KML_Tour.kml")
...
def time_range_slider(in_df):
 ## Range selector
    ...
    try:
        ...
        slider = st.slider('Seleccionar Fecha (YYYY-MM-DD)', min_value=start_date, value=(start_date,end_date) ,max_value=end_date, format=format)
...
def time_filter(in_df, date_column):
    try:
        ...
        with col1:
                start_date = st.date_input("Fecha de Inicio",value=starting)     
        with col2:
                start_time = st.time_input("Hora de Inicio", value=datetime.time(0, 00, 00))
        with col3:
                end_date = st.date_input("Fecha de Finalización",value=ending)
        with col4:
                end_time = st.time_input("Hora de Finalización", value=datetime.time(23, 59, 59))
        ...
        st.markdown(":orange[Registros Filtrados:]  " + str(len(selected_time.index)))
        st.table(selected_time)
        return selected_time
    except parser.ParserError:
        st.error("Seleccione la columna con datos de Fecha/Hora.")
...
def declutterer(in_df, date_column):
    ...
    Time_intervals = { "Año": "A", "Mes": "M", "Semana": "W", "Día": "D", "Hora" : "H", "Minuto": "T", "Segundo": "S"}    
    spot1, spot2, spot3, spot4 = st.columns(4)
    with spot1:
        count_of_time = st.number_input("Conteo de Intervalos", min_value=1, max_value=60,)
    with spot2:
        choose_interval = st.selectbox("Tipo de Intervalo", options=list(Time_intervals.keys()))
    ...
    st.markdown(":orange[Número de Ubicaciones Promediadas:]  " + str(len(declutter_data.index)))
...
if uploaded_file != None:
    with st.expander("Gestionar Datos"):
        ...
        with tabii:
            filterbytime = st.checkbox("Filtrar por Fecha/Hora")
            if filterbytime == True:
                datetime_Column = st.selectbox("Seleccione la Columna de Fecha/Hora", options=preview_data.columns)
                preview_data = time_filter(preview_data,date_column=datetime_Column)
                
        with tabiii:
            declutter_dis = st.checkbox("Consolidar puntos a una ubicación promedio basada en el intervalo de tiempo seleccionado.")
            if declutter_dis == True:
                try:
                    datetim_Column = st.selectbox("Elija la Columna de Fecha/Hora", options=preview_data.columns)
                except AttributeError:
                    st.error("Seleccione la columna con datos de Fecha/Hora.")
                try:
...
if uploaded_file == None:
    tabA, tabB = st.tabs(["Crear Geovalla", "Mapeo de Direcciones IP"])
    with tabA:
        make_geofence_map()
    with tabB:
        make_IPaddress_Map()
...
if uploaded_file != None:
    if declutter_dis or filterbytime == True:
        st.markdown(":orange[LOS FILTROS ESTÁN ACTIVOS]")
    tab1, tab2, tab3 = st.tabs(["Vista Previa/Mapa KML", "Mapas de Análisis", "Crear Geovalla"])
    with tab1:
        try:
            st.map(preview_data)
            st.download_button("Descargar como CSV", data=preview_data.to_csv(), file_name="Fetch_CSV_Export.csv")
        except TypeError:
    ...
    with tab2:
        try:
            ...
            st.error("Verifique que sus datos tengan columnas de Latitud y Longitud")
    with tab3:
        make_geofence_map()
...
with st.expander("Declaración de Privacidad y Uso de API"):
    st.write("""
    ## Declaración de Privacidad de Datos e Información sobre el Uso de la API
    
    Nos tomamos su privacidad en serio. North Loop Consulting solo tendrá acceso temporal a pequeñas porciones de datos que puedan estar involucradas en la generación de informes de errores.  
Estos datos no se almacenarán por un período más largo del necesario para identificar y corregir cualquier problema, si es que se almacenan.
            
Utilizamos una empresa externa, Streamlit, para proporcionar alojamiento para Fetch.  
Para obtener más información sobre Streamlit y su manejo de los datos de los usuarios, consulte su [Política de Privacidad](https://streamlit.io/privacy-policy).

Para obtener información específica sobre cómo Streamlit maneja los conjuntos de datos que ha subido como archivos, consulte sus 
[documentos sobre la carga de archivos](https://docs.streamlit.io/knowledge-base/using-streamlit/where-file-uploader-store-when-deleted).

Siempre es mejor evitar subir datos sensibles a cualquier servicio en línea. Por favor, no suba ningún dato que pueda contener información personal identificable o datos sensibles.

Fetch está destinado a ser una herramienta gratuita disponible para todos. Para hacer esto, se utilizan recursos gratuitos para devolver información como datos de direcciones IP. Hay límites en este uso gratuito. Puede encontrarse con períodos en los que se haya alcanzado ese límite y estos recursos no estén disponibles. Si desea explorar el apoyo financiero para la herramienta para expandir estas capacidades, estoy abierto a la conversación. Simplemente póngase en contacto a través de la página de contacto en nuestro sitio.   
""")
st.markdown(":orange[© 2024 North Loop Consulting - Fetch_v3.10]")
